...
app.get('/api/galeria/:id', async (req, res) => {
    try {
        ...
        // ==========================================
        // CORREÇÃO AQUI: Inverte a lista de subpastas vinda do Google Drive
        // ==========================================
        const listaSubpastasInvertida = subpastas.data.files ? [...subpastas.data.files].reverse() : [];

        const secoes = await Promise.all(listaSubpastasInvertida.map(async (sub) => {
